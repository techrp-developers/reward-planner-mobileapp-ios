...
describe("RNViewShot.web", () => {
    ...
    describe("captureRef", () => {
        ...
        it("returns data-uri for tmpfile result (with warning)", async () => {
            ...
            expect(result).toBe("data:image/png;base64,abc123");
            expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("Tmpfile is not implemented"));
            warnSpy.mockRestore();
        });
        it("resizes canvas when width and height are provided", async () => {
            ...
            expect(mockContext.drawImage).toHaveBeenCalledWith(mockCanvas, 0, 0, 200, 300);
            spy.mockRestore();
        });
    });
...
//# sourceMappingURL=RNViewShot.web.test.js.map
...
describe("ViewShot component", () => {
    ...
    it("renders with collapsable={false} and onLayout", async () => {
        let tree;
        await act(async () => {
            tree = renderer.create(_jsx(ViewShot, { children: _jsx("span", { children: "Hello" }) }));
        });
    ...
    it("passes style prop to the outer View", async () => {
        ...
        let tree;
        await act(async () => {
            tree = renderer.create(_jsx(ViewShot, { style: style }));
        });
        const json = tree.toJSON();
    ...
    it("warns when captureMode is set without onCapture", async () => {
        ...
        let tree;
        await act(async () => {
            tree = renderer.create(_jsx(ViewShot, { captureMode: "mount" }));
        });
        expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("onCapture prop callback is missing"));
        warnSpy.mockRestore();
        await act(async () => tree.unmount());
    });
    it("warns when continuous mode uses non-tmpfile result", async () => {
        ...
        let tree;
        await act(async () => {
            tree = renderer.create(_jsx(ViewShot, { captureMode: "continuous", onCapture: jest.fn(), options: { result: "base64" } }));
        });
        expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("result=tmpfile is recommended"));
        warnSpy.mockRestore();
        await act(async () => tree.unmount());
    });
    it("warns when update mode uses non-tmpfile result", async () => {
        ...
        let tree;
        await act(async () => {
            tree = renderer.create(_jsx(ViewShot, { captureMode: "update", onCapture: jest.fn(), options: { result: "data-uri" } }));
        });
        expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("result=tmpfile is recommended"));
        warnSpy.mockRestore();
        await act(async () => tree.unmount());
    ...
    it("does not warn when captureMode is undefined with onCapture", async () => {
        ...
        let tree;
        await act(async () => {
            tree = renderer.create(_jsx(ViewShot, { onCapture: jest.fn() }));
        });
        const viewShotWarnings = warnSpy.mock.calls.filter(call => typeof call[0] === "string" &&
    ...
    it("does not warn for valid continuous + onCapture + tmpfile", async () => {
        ...
        let tree;
        await act(async () => {
            tree = renderer.create(_jsx(ViewShot, { captureMode: "continuous", onCapture: jest.fn(), options: { result: "tmpfile" } }));
        });
        const viewShotWarnings = warnSpy.mock.calls.filter(call => typeof call[0] === "string" &&
            call[0].includes("react-native-view-shot"));
        expect(viewShotWarnings).toHaveLength(0);
    ...
    it("starts requestAnimationFrame loop for continuous mode", async () => {
        ...
        let tree;
        await act(async () => {
            tree = renderer.create(_jsx(ViewShot, { captureMode: "continuous", onCapture: jest.fn() }));
        });
    ...
    it("capture() waits for first layout before calling captureRef", async () => {
        let tree;
        await act(async () => {
            tree = renderer.create(_jsx(ViewShot, { captureMode: "mount", onCapture: jest.fn() }));
        });
    ...
    it("re-syncs capture loop when captureMode changes", async () => {
        ...
        rafSpy.mockClear();
        await act(async () => {
            tree.update(_jsx(ViewShot, { captureMode: "continuous", onCapture: jest.fn() }));
        });
        expect(rafSpy).toHaveBeenCalled();
        await act(async () => tree.unmount());
        rafSpy.mockRestore();
    ...
    it("captures on update when captureMode is 'update'", async () => {
        let tree;
        await act(async () => {
            tree = renderer.create(_jsx(ViewShot, { captureMode: "update", onCapture: jest.fn() }));
        });
        ...
        await act(async () => {
            tree.update(_jsx(ViewShot, { captureMode: "update", onCapture: jest.fn(), options: { quality: 0.5 } }));
        });
    ...
    it("calls onCapture callback on successful capture", async () => {
        ...
        let tree;
        await act(async () => {
            tree = renderer.create(_jsx(ViewShot, { captureMode: "update", onCapture: onCapture }));
        });
        // Trigger layout
        const json = tree.toJSON();
        await act(async () => {
        ...
        // Trigger update
        await act(async () => {
            tree.update(_jsx(ViewShot, { captureMode: "update", onCapture: onCapture, options: { quality: 0.8 } }));
        });
        await new Promise(r => setTimeout(r, 50));
    ...
    it("forwards ref to the inner View node with capture() attached", async () => {
        ...
        let tree;
        await act(async () => {
            tree = renderer.create(_jsx(ViewShot, { ref: ref }));
        });
    ...
    it("supports a function ref forwarded to the inner View", async () => {
        ...
        let tree;
        await act(async () => {
            tree = renderer.create(_jsx(ViewShot, { ref: refFn }));
        });
    ...
    it("forwards onLayout to props.onLayout", async () => {
        ...
        let tree;
        await act(async () => {
            tree = renderer.create(_jsx(ViewShot, { onLayout: onLayout }));
        });
        const json = tree.toJSON();
        const event = {
...
//# sourceMappingURL=ViewShot.test.js.map
...
import { isIOS, isOther, isNull, isObject, isFunction, isString, isUndefined } from "../../common/index.js";
 ...
 * @param fn
 */
export function addOnAppDestroy(fn) {
    ...
    try {
      for (let i = 0; i < onAppDestroyCallbacks.length; i++) {
        try {
          ...
          console.warn(`Firebase: an onAppDestroy callback (index ${i}) for app '${name}' threw and was ` + 'skipped; remaining callbacks will still run. Inspect the offending listener:', e);
        }
      }
    } finally {
      delete APP_REGISTRY[name];

...
import { addOnAppDestroy, getApp } from "./app.js";
...
const MODULAR_INSTANCES = {};
let destroyHookRegistered = false;
function ensureDestroyHook() {
 ...
 */
export function getOrCreateModularInstance(ModuleClass, config, app, customUrlOrRegion) {
  ensureDestroyHook();
  ...
  if (!hasMultiAppSupport && _app.name !== DEFAULT_APP_NAME) {
    throw new Error([`You attempted to call "${namespace}" with a secondary Firebase App but; ${namespace} does not support multiple Firebase Apps.`, '', `Ensure you access ${namespace} from the default application only.`].join('\r\n'));
  }
  ...
  if (!MODULAR_INSTANCES[_app.name][key]) {
    MODULAR_INSTANCES[_app.name][key] = new ModuleClass(_app, Object.assign({}, config), customUrlOrRegion);
  }
  return MODULAR_INSTANCES[_app.name][key];
}

 ...
 *
 */
import { APP_NATIVE_MODULE, UTILS_NATIVE_MODULE } from "../constants.js";
 ...
 */
function createRoutingCompositeProxy(namespace, hosts, isTurboModule) {
  const routingMap = {};
  const memoizedMethods = {};
  const memoizedConstants = {};
  for (const {
    module,
    argToPrepend
  } of hosts) {
  ...
  return new Proxy(target, {
    get(_target, name) {
      ...
      if (!memoizedMethods[name]) {
        memoizedMethods[name] = nativeModuleMethodWrapped(namespace, value.bind(route.host), route.argToPrepend, isTurboModule);
      }
 ...
 */
function createSingleHostComposite(namespace, moduleName, argToPrepend = [], isTurboModule = true) {
  const nativeModule = getReactNativeModule(moduleName);
  if (!nativeModule) {
    throw new Error(getMissingModuleHelpText(namespace));
  }
  return createRoutingCompositeProxy(namespace, [{
    module: nativeModule,
    argToPrepend
  }], isTurboModule);
}
 ...
 */
export function getStaticUtilsModule() {
